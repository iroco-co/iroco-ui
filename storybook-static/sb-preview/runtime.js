		...
		module.exports = function inspect_(obj, options2, depth, seen) {
			...
			if (!isDate(obj) && !isRegExp(obj)) {
				var ys = arrObjKeys(obj, inspect),
					...
					stringTag =
						!isPlainObject3 && toStringTag && Object(obj) === obj && toStringTag in obj
	...
	'../../node_modules/qs/lib/stringify.js'(exports, module) {
		...
		var getSideChannel = require_side_channel(),
			...
			stringify2 = function stringify3(
				...
				for (
					var adjustedPrefix =
							commaRoundTrip && isArray2(obj) && obj.length === 1 ? prefix2 + '[]' : prefix2,
						j = 0;
					j < objKeys.length;
					++j
				) {
	...
	'../../node_modules/qs/lib/parse.js'(exports, module) {
		...
		var utils = require_utils(),
			...
			parseArrayValue = function (val, options2) {
				return val && typeof val == 'string' && options2.comma && val.indexOf(',') > -1
					? val.split(',')
					: val;
			},
			...
			normalizeParseOptions = function (opts) {
				...
				return {
					...
					allowSparse:
						typeof opts.allowSparse == 'boolean' ? opts.allowSparse : defaults.allowSparse,
...
var require_getRawTag = __commonJS({
	'../../node_modules/lodash/_getRawTag.js'(exports, module) {
		...
		function getRawTag2(value2) {
			...
			return (
				unmasked && (isOwn ? (value2[symToStringTag3] = tag) : delete value2[symToStringTag3]),
				result2
			);
		}
...
var require_isLength = __commonJS({
	'../../node_modules/lodash/isLength.js'(exports, module) {
		...
		function isLength(value2) {
			return (
				typeof value2 == 'number' && value2 > -1 && value2 % 1 == 0 && value2 <= MAX_SAFE_INTEGER
			);
		}
...
var require_baseIsTypedArray = __commonJS({
	'../../node_modules/lodash/_baseIsTypedArray.js'(exports, module) {
		...
		function baseIsTypedArray(value2) {
			return (
				isObjectLike2(value2) && isLength(value2.length) && !!typedArrayTags[baseGetTag2(value2)]
			);
		}
...
var require_isSymbol = __commonJS({
	'../../node_modules/lodash/isSymbol.js'(exports, module) {
		...
		function isSymbol2(value2) {
			return (
				typeof value2 == 'symbol' || (isObjectLike2(value2) && baseGetTag2(value2) == symbolTag2)
			);
		}
		...
		function overRest(func, start2, transform) {
			return (
				...
				function () {
					...
					return (otherArgs[start2] = transform(array)), apply(func, this, otherArgs);
				}
			);
		}
	...
	'../../node_modules/synchronous-promise/index.js'(exports, module) {
		...
		SynchronousPromise4.prototype = {
			then: function (nextFn, catchFn) {
				...
				if (this._isRejected()) {
					if (this._paused)
						return this._continuations.push({ promise: next, nextFn, catchFn }), next;
					if (catchFn)
						try {
				...
				return (
					this._continuations.push({ promise: next, nextFn, catchFn }), this._runResolutions(), next
				);
			},
			catch: function (handler) {
				if (this._isResolved()) return SynchronousPromise4.resolve(this._data)._setParent(this);
				var next = SynchronousPromise4.unresolved()._setParent(this);
				return (
					this._continuations.push({ promise: next, catchFn: handler }), this._runRejections(), next
				);
			},
				...
				args2.length
					? new SynchronousPromise4(function (resolve, reject) {
							var allErrors = [],
								numRejected = 0,
								doReject = function () {
									numRejected === args2.length && reject(createAggregateErrorFrom(allErrors));
								},
								resolved = !1,
				...
				args2.length
					? new SynchronousPromise4(function (resolve) {
							var allData = [],
								numSettled = 0,
								doSettled = function () {
									(numSettled += 1), numSettled === args2.length && resolve(allData);
								};
							args2.forEach(function (arg, idx) {
								SynchronousPromise4.resolve(arg)
									.then(function (thisResult) {
	...
	'../../node_modules/lodash/_baseKeysIn.js'(exports, module) {
		...
		function baseKeysIn(object) {
			...
			for (var key2 in object)
				(key2 == 'constructor' && (isProto || !hasOwnProperty5.call(object, key2))) ||
					result2.push(key2);
			return result2;
		}
						...
						(l = [
							{
								...
								value: function (e2) {
									var t3,
										...
										b2 = i();
									p === 'chrome' &&
										typeof (b2 == null || (t3 = b2.brave) === null || t3 === void 0
											? void 0
											: t3.isBrave) == 'function' &&
										(p = 'brave'),
										p && (s2[p] = !0),
...
function getRawTag(value2) {
	...
	return (
		unmasked && (isOwn ? (value2[symToStringTag] = tag) : delete value2[symToStringTag]), result2
	);
}
	...
	PostMessageTransport = class {
		...
		send(event, options2) {
			let {
				...
				query = import_qs.default.parse(location?.search || '', { ignoreQueryPrefix: !0 }),
				data = stringify({ key: KEY, event, refId: query.refId }, stringifyOptions);
...
function createChannel2({ url, async = !1, onError = (err) => logger.warn(err) }) {
	let channelUrl = url;
	if (!channelUrl) {
...
var __create3 = Object.create,
	...
	__commonJS3 = (cb, mod) =>
		function () {
			return (
				mod || (0, cb[__getOwnPropNames3(cb)[0]])((mod = { exports: {} }).exports, mod), mod.exports
			);
		},
		...
		return matches2
			? arr.concat(
					matches2.map(function (match) {
						var _a, _b;
						return (_b =
							(_a = match.match(/[\t ]/g)) === null || _a === void 0 ? void 0 : _a.length) !==
							null && _b !== void 0
							? _b
							: 0;
					})
	...
	if (foo && bar && (ctor = foo.constructor) === bar.constructor) {
		...
		if (ctor === Set) {
			...
			for (len of foo)
				if (
					((tmp = len),
					(tmp && typeof tmp == 'object' && ((tmp = find(bar, tmp)), !tmp)) || !bar.has(tmp))
				)
					return !1;
			return !0;
		}
...
var B = Object.create,
	...
	v = (r, e, n) => (
		(n = r != null ? B(h(r)) : {}),
		E(e || !r || !r.__esModule ? R(n, 'default', { value: r, enumerable: !0 }) : n, r)
	),
	x = I((T) => {
						...
						switch (A.slice(8, -1)) {
							...
							case 'Array':
								...
								for (d = 0; d < o.length; d++)
									if ((d in o || d in s) && (d in o != d in s || !i(o[d], s[d], p))) return !1;
								return !0;
							case 'Object':
								return i(e(o), e(s), p);
							default:
...
var inferType = (value2, name2, visited) => {
	...
	inferArgTypes = (context) => {
		let { id, argTypes: userArgTypes = {}, initialArgs = {} } = context,
			...
			userArgTypesNames = (0, import_mapValues.default)(userArgTypes, (argType, key2) => ({
				name: key2
			}));
...
var matches = (name2, descriptor) =>
		Array.isArray(descriptor) ? descriptor.includes(name2) : name2.match(descriptor),
	...
	StoryStore = class {
		constructor() {
			(this.getStoriesJsonData = () => {
				...
				(this.hooks = {}),
				(this.processCSFFileWithCache = (0, import_memoizerific2.default)(CSF_CACHE_SIZE)(
					processCSFFile
				)),
				(this.prepareMetaWithCache = (0, import_memoizerific2.default)(CSF_CACHE_SIZE)(
					prepareMeta
				)),
				(this.prepareStoryWithCache = (0, import_memoizerific2.default)(STORY_CACHE_SIZE)(
					prepareStory
				)),
				(this.initializationPromise = new import_synchronous_promise.SynchronousPromise(
					(resolve) => {
		...
		initialize({ storyIndex, importFn, cache = !1 }) {
			return (
				...
				cache ? this.cacheAllCSFFiles() : import_synchronous_promise.SynchronousPromise.resolve()
			);
		}
		...
		loadAllCSFFiles({ batchSize = EXTRACT_BATCH_SIZE } = {}) {
				...
				loadInBatches = (remainingImportPaths) => {
					...
					let csfFilePromiseList = remainingImportPaths
						...
						.map(([importPath, storyId]) =>
							this.loadCSFFileByStoryId(storyId).then((csfFile) => ({ importPath, csfFile }))
						);
			...
			return loadInBatches(importPaths).then((list) =>
				list.reduce((acc, { importPath, csfFile }) => ((acc[importPath] = csfFile), acc), {})
			);
		}
		cacheAllCSFFiles() {
...
var userOrAutoTitleFromSpecifier = (fileName, entry, userTitle) => {
    ...
    `);
		...
		if (importPathMatcher.exec(normalizedFileName)) {
			if (!userTitle) {
				...
				return (
					(path = stripExtension(path)), (path = removeRedundantFilename(path)), path.join('/')
				);
			}
	...
	storySort =
		...
		(a, b2) => {
			...
			options2.includeNames && (storyTitleA.push(a.name), storyTitleB.push(b2.name));
			let depth = 0;
			for (; storyTitleA[depth] || storyTitleB[depth]; ) {
				...
				if (nameA !== nameB) {
					...
					return indexA !== -1 || indexB !== -1
						? (indexA === -1 &&
								(indexWildcard !== -1 ? (indexA = indexWildcard) : (indexA = order.length)),
						  indexB === -1 &&
								(indexWildcard !== -1 ? (indexB = indexWildcard) : (indexB = order.length)),
						  indexA - indexB)
						: method === 'configure'
						? 0
...
var import_synchronous_promise2 = __toESM(require_synchronous_promise(), 1),
	...
	StoryStoreFacade = class {
		...
		getStoryIndex(store) {
			let fileNameOrder = Object.keys(this.csfExports),
				storySortParameter = this.projectAnnotations.parameters?.options?.storySort,
        ...
        legacy (6.x) MDX API, or use the new store.`);
			...
			let { default: defaultExport, __namedExportsOrder, ...namedExports } = fileExports,
				{ id: componentId, title, tags: componentTags = [] } = defaultExport || {},
			...
			let storyExports = Object.entries(sortedExports).filter(([key2]) => G(key2, defaultExport)),
				docsOptions = scope.DOCS_OPTIONS || {},
				{ autodocs } = docsOptions,
				componentAutodocs = componentTags.includes(AUTODOCS_TAG),
				autodocsOptedIn = autodocs === !0 || (autodocs === 'tag' && componentAutodocs);
			...
			storyExports.forEach(([key2, storyExport]) => {
				let exportName = M(key2),
					id = storyExport.parameters?.__id || N(componentId || title, exportName),
					name2 =
	...
	addDecorator = (decorator) => {
		checkMethod('addDecorator'), scope.__STORYBOOK_CLIENT_API__?.addDecorator(decorator);
	},
	addParameters = (parameters) => {
		checkMethod('addParameters'), scope.__STORYBOOK_CLIENT_API__?.addParameters(parameters);
	},
	addLoader = (loader) => {
	...
	addArgTypes = (argTypes) => {
		checkMethod('addArgTypes'), scope.__STORYBOOK_CLIENT_API__?.addArgTypes(argTypes);
	},
	addArgsEnhancer = (enhancer) => {
		checkMethod('addArgsEnhancer'), scope.__STORYBOOK_CLIENT_API__?.addArgsEnhancer(enhancer);
	},
	...
	addStepRunner = (stepRunner) => {
		checkMethod('addStepRunner'), scope.__STORYBOOK_CLIENT_API__?.addStepRunner(stepRunner);
	},
		...
		constructor({ storyStore } = {}) {
			(this.lastFileName = 0),
				...
				(this.storiesOf = (kind, m2) => {
					...
					let baseFilename = m2 && m2.id ? `${m2.id}` : (this.lastFileName++).toString(),
					...
					let meta = { id: L(kind), title: kind, decorators: [], loaders: [], parameters: {} };
...
Read more here: https://github.com/storybookjs/storybook/blob/master/MIGRATION.md#can-no-longer-add-decoratorsparameters-after-stories`);
	...
	getQueryParams = () => {
		...
		return document4 && document4.location && document4.location.search
			? (0, import_qs2.parse)(document4.location.search, { ignoreQueryPrefix: !0 })
			: {};
	},
	...
	require_encode = __commonJS3({
		'../../node_modules/entities/lib/encode.js'(exports) {
			...
			function singleCharReplacer(c2) {
				return (
					'&#x' +
					(c2.length > 1 ? getCodePoint(c2) : c2.charCodeAt(0)).toString(16).toUpperCase() +
					';'
				);
			}
...
var StoryRender = class {
	...
	Preview = class {
		...
		initialize({ getStoryIndex, importFn, getProjectAnnotations }) {
			return (
				...
				(this.importFn = importFn),
				this.setupListeners(),
				this.getProjectAnnotationsOrRenderError(getProjectAnnotations).then((projectAnnotations) =>
			...
			return import_synchronous_promise3.SynchronousPromise.resolve()
				...
				.then((projectAnnotations) => {
					if (
						...
						(this.renderToCanvas =
							projectAnnotations.renderToCanvas || projectAnnotations.renderToDOM),
						!this.renderToCanvas)
					)
						throw new Error(dedent`
		...
		initializeWithProjectAnnotations(projectAnnotations) {
			...
			if (scope.FEATURES?.storyStoreV7) storyIndexPromise = this.getStoryIndexFromServer();
			else {
				if (!this.getStoryIndex) throw new Error('No `getStoryIndex` passed defined in v6 mode');
				storyIndexPromise = import_synchronous_promise3.SynchronousPromise.resolve().then(
					this.getStoryIndex
				);
			}
		...
		async onStoryIndexChanged() {
			if ((delete this.previewEntryError, !!this.storyStore.projectAnnotations))
				try {
					...
					this.storyStore.storyIndex || (await this.initializeWithStoryIndex(storyIndex)),
		...
		async onUpdateArgs({ storyId, updatedArgs }) {
			this.storyStore.args.update(storyId, updatedArgs),
				...
				this.channel.emit(STORY_ARGS_UPDATED, { storyId, args: this.storyStore.args.get(storyId) });
		}
      ...
      Do you have an error in your \`preview.js\`? Check your Storybook's browser console for errors.`);
			return (
				scope.FEATURES?.storyStoreV7 && (await this.storyStore.cacheAllCSFFiles()),
	...
	DocsContext = class {
		...
		resolveAttachedModuleExportType(moduleExportType) {
			...
			if (moduleExportType === 'meta') return { type: 'meta', csfFile: this.attachedCSFFile };
...
var PreviewWithSelection = class extends Preview {
		...
		async renderSelection({ persistedArgs } = {}) {
			...
			if (
				...
				if (!render.story) throw new Error('Render has not been prepared!');
				let { parameters, initialArgs, argTypes, unmappedArgs } = this.storyStore.getStoryContext(
					render.story
				);
				scope.FEATURES?.storyStoreV7 &&
	...
	parseArgsParam = (argsString) => {
		...
		return Object.entries(import_qs3.default.parse(parts.join(';'), QS_OPTIONS)).reduce(
			(acc, [key2, value2]) =>
				validateArgs(key2, value2)
...
var getQueryString = ({ selection, extraParams }) => {
		let { search = '' } = document3.location,
			{ path, selectedKind, selectedStory, ...rest } = import_qs3.default.parse(search, {
				ignoreQueryPrefix: !0
			});
	...
	getSelectionSpecifierFromPath = () => {
		let query = import_qs3.default.parse(document3.location.search, { ignoreQueryPrefix: !0 }),
	...
	WebView = class {
		constructor() {
			this.testing = !1;
			let { __SPECIAL_TEST_PARAMETER__ } = import_qs3.default.parse(document22.location.search, {
				ignoreQueryPrefix: !0
			});
...
function start(renderToCanvas, { decorateStory: decorateStory2, render } = {}) {
	if ((scope && (scope.IS_STORYBOOK = !0), FEATURES?.storyStoreV7))
		return {
			...
			clientApi: { storiesOf: removedApi('clientApi.storiesOf'), raw: removedApi('raw') }
		};
	...
	return (
			...
			(scope.__STORYBOOK_STORY_STORE__ = preview.storyStore)),
		{
			...
			configure(renderer, loadable, m2, disableBackwardCompatibility = !0) {
				if (disableBackwardCompatibility) throw new Error('unexpected configure() call');
